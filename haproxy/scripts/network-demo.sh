...
# Image to use:
IMAGE=openqsafe/haproxy-alpine
...
docker run -v `pwd`/oqs-root:/opt/haproxy/root --entrypoint /opt/haproxy/sh.sh -it $IMAGE openssl req -x509 -new -newkey $SIG_ALG -keyout /opt/haproxy/root/CA.key -out /opt/haproxy/root/CA.crt -subj "/CN=OQS-HAproxy-CA" -days 365 -config /opt/oqssa/ssl/openssl.cnf

mkdir -p oqs-haproxy
# Mount local folder oqs-haproxy to store generated server key and CSR:
docker run -v `pwd`/oqs-haproxy:/opt/haproxy/server --entrypoint /opt/haproxy/sh.sh -it $IMAGE openssl req -new -newkey $SIG_ALG -keyout /opt/haproxy/server/server.key -out /opt/haproxy/server/server.csr -nodes -subj "/CN=my.ha.proxy" -config /opt/oqssa/ssl/openssl.cnf

# Mount both root CA and haproxy folders to generate haproxy server certificate:
docker run -v `pwd`/oqs-root:/opt/haproxy/root -v `pwd`/oqs-haproxy:/opt/haproxy/server --entrypoint /opt/haproxy/sh.sh -it $IMAGE openssl x509 -req -in /opt/haproxy/server/server.csr -out /opt/haproxy/server/server.crt -CA /opt/haproxy/root/CA.crt -CAkey /opt/haproxy/root/CA.key -CAcreateserial -days 365
...
docker run -p 4443:4443 --network haproxy-demo-net --name my.ha.proxy -v `pwd`/oqs-haproxy:/opt/haproxy/conf -e SIG_ALG=$SIG_ALG -e KEM_ALG=$KEM_ALG -t $IMAGE &

cd oqs-root

# --- Demo code to build appliance with newly created CA cert baked in
#echo "FROM $IMAGE" > Dockerfile-setca
#echo "ADD CA.crt /opt/haproxy/conf/CA.crt" >> Dockerfile-setca
...
# Start image in reverse proxy mode with proper root cert mounted in:
docker run -p $LOCALPORT:8080 -v `pwd`:/opt/haproxy/conf --name ha-localproxy --entrypoint /opt/haproxy/client/startup.sh --network haproxy-demo-net --rm -e SIG_ALG=$SIG_ALG -e KEM_ALG=$KEM_ALG -t $IMAGE &
